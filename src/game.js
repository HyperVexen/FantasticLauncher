// game.js - ENHANCED WITH CRASH DETECTION AND AUTO-REPAIR
const fs = require('fs');
...
class Game {
  constructor() {
    ...
    this.crashCallbacks = {
      ...
      onRepairStarted: null,
      onRepairCompleted: null,
      onGameExit: null
    };
  ...
  setLauncherPaths(launcher) {
    ...
    crashDetector.initialize(launcher);
    
    // Set up crash detection callbacks
    this.setupCrashDetectionCallbacks();
  }
  
  // Set up crash detection callbacks
  setupCrashDetectionCallbacks() {
    // Handle crash detection
    crashDetector.onCrashDetected = (crashReport) => {
      this.log(`Game crash detected: ${crashReport.type} - ${crashReport.details}`, true);
      
      // Notify the main process about the crash
      if (this.crashCallbacks.onCrashDetected) {
        this.crashCallbacks.onCrashDetected(crashReport);
      }
    };
    
    // Handle repair start
    crashDetector.onRepairStarted = (repairInfo) => {
      this.log(`Auto-repair started (attempt ${repairInfo.attempt}/${repairInfo.maxAttempts})`);
      
      if (this.crashCallbacks.onRepairStarted) {
        this.crashCallbacks.onRepairStarted(repairInfo);
      }
    };
    
    // Handle repair completion
    crashDetector.onRepairCompleted = (repairResult) => {
      this.log(`Auto-repair completed - Success: ${repairResult.success}`);
      
      if (this.crashCallbacks.onRepairCompleted) {
        this.crashCallbacks.onRepairCompleted(repairResult);
      }
    };
    
    // Handle game exit
    crashDetector.onGameExit = (exitInfo) => {
      this.log(`Game process exited - Crash: ${exitInfo.isCrash}, Play time: ${exitInfo.playTime}s`);
      
      if (this.crashCallbacks.onGameExit) {
        this.crashCallbacks.onGameExit(exitInfo);
      }
    };
  }
  
  // Set crash detection callbacks (for external use)
  setCrashCallbacks(callbacks) {
    this.crashCallbacks = { ...this.crashCallbacks, ...callbacks };
  }

  // Extract native libraries (keeping existing implementation)
  ...
  // Build classpath for game (keeping existing implementation)
  ...
  // Helper method to find all JARs (keeping existing)
  ...
  // Add libraries to classpath (keeping existing)
  ...
  // Get the path to the client JAR file (keeping existing)
  ...
  // Build JVM arguments (keeping existing)
  ...
  // Build game arguments (keeping existing implementation)
  ...
  // Replace game arg placeholders (keeping existing)
  ...
  // Check if argument conditions are met (keeping existing)
  ...
  // Generate UUID from username (keeping existing)
  ...
  // Get the native OS name for Minecraft (keeping existing)
  getNativeOS() {
  ...
  // Find Java executable (keeping existing)
  async findJava() {
  ...
  // ENHANCED: Launch the game with crash detection
  async launchGame(username, minecraftVersion, fabricVersion, ram) {
    try {
      ...
      // Launch the game with crash detection
      ...
      const monitoringResult = crashDetector.startMonitoring(
        gameProcess, 
        username, 
        minecraftVersion, 
        fabricVersion
      ...
      this.log(`Started crash monitoring for PID: ${monitoringResult.pid}`);
      
      return new Promise((resolve, reject) => {
        gameProcess.on('error', (err) => {
          this.log(`Game process error: ${err.message}`, true);
          reject(err);
        });
        
        gameProcess.once('spawn', () => {
          this.log(`Game launched successfully with PID: ${gameProcess.pid}`);
          resolve({ 
            pid: gameProcess.pid, 
            success: true,
            crashMonitoring: true,
            monitoringDetails: monitoringResult
          });
  ...
  // NEW METHOD: Ensure virtual assets are properly set up (keeping existing implementation)
  ...
  // NEW: Get crash detector instance
  getCrashDetector() {
    return crashDetector;
  }
  
  // NEW: Trigger manual repair
  async triggerRepair(minecraftVersion, fabricVersion) {
    this.log("Manual repair triggered from game module");
    return await crashDetector.triggerManualRepair(minecraftVersion, fabricVersion);
  }
  
  // NEW: Get crash statistics
  getCrashStatistics() {
    return crashDetector.getCrashStatistics();
  }
  
  // NEW: Get last crash report
  getLastCrashReport() {
    return crashDetector.getLastCrashReport();
  }
  
  // NEW: Check if repair is available
  canRepair() {
    return crashDetector.canRepair();
  }
  
  // NEW: Reset repair attempts
  resetRepairAttempts() {
    crashDetector.resetRepairAttempts();
  }
  
  // NEW: Set auto-repair enabled/disabled
  setAutoRepair(enabled) {
    crashDetector.setAutoRepair(enabled);
  }
  
  // NEW: Stop crash monitoring
  stopCrashMonitoring() {
    crashDetector.stopMonitoring();
  }
  
  // NEW: Cleanup crash detector
  cleanupCrashDetector() {
